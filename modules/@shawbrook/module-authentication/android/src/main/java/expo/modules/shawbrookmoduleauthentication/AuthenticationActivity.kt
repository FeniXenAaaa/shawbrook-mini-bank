...
class AuthenticationActivity : AppCompatActivity(), KoinComponent {
      ...
      object : BiometricPrompt.AuthenticationCallback() {
        ...
        override fun onAuthenticationFailed() {
          ...
          if (biometricAttempts >= MAX_BIOMETRIC_ATTEMPTS) {
            ...
            Toast.makeText(this@AuthenticationActivity, "Biometric failed, try again", Toast.LENGTH_SHORT).show()
          }
        }
  ...
  override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
    ...
    if (requestCode == 1001) {
      val pinSuccess = data?.getBooleanExtra("pinSuccess", false) ?: false
      finishWithResult(pinSuccess)
    }
  }
  ...
  private fun signInWithEmailPassword(email: String, password: String): String? {
    //TODO: URLs should be based on env
    return try {
